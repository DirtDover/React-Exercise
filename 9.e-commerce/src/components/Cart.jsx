...
export default function Cart({onClose}) {
    ...
    className='fixed z-10 inset-0 bg-slate-700/75 flex justify-center items-center'>
        ...
        className="z-20 relative bg-slate-300 text-slate-900 md:min-w-[700px] px-10 pt-10 pb-6 rounded border border-slate-600 mb-[10vh]"
        >
            <button
            onClick={onClose}
           ...
           <p className="text-xl">Your Total : <span
           className='font-semibold '>{cart.cartItems.reduce((acc, curr) => acc + curr.price * curr.quantity, 0).toFixed(2)}$</span></p>
           <button 
           className='block  mx-auto bg-slate-800 text-slate-200 rounded px-4 py-2 mt-7'
